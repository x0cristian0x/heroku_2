...
pregnationServer <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    autor = eventReactive( input$click_pregnation, {
      ifelse (input$autor =="Chile 2019", "Chile_2019", "Atalah")   })
...
# Seccion Interpretation ===========
    ...
    output$read_pregnation <- renderText({  
      ...
      func_pregnation(
        bmi_pregnation = bmi_pregnation(),
        obesidad = tablePregnation[ tablePregnation$Week==week_pregnation(), 3],
        sobrepeso = tablePregnation[ tablePregnation$Week==week_pregnation(), 4],
        normal = tablePregnation[ tablePregnation$Week==week_pregnation(), 5]
        )
