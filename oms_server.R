...
# Seccion Input ==========    
    ...
    sex = eventReactive( input$click_0_19,{
              ifelse (input$sex=="Hombre", "Boy", "Girl")
                  })

    distribution = eventReactive( input$click_0_19, {
              ifelse (input$distribution =="Z-score","z", "p")
                 })

    date_born = eventReactive( input$click_0_19, {
      date_born = ifelse( input$months >0, input$months,
        round( time_length( as_date(Sys.time()) - as_date(input$calendar),
                            unit = "month")) )
              })
...
# Seccion Interpretation ===========
        ...
        output$waterlow <- renderText({
          ...
          waterlow(date_born = date_born(), height = height(), 
                   weight = weight(), height_median = table_1()[["Median"]], 
                   weight_median = table_2()[["Median"]] )
          })        
